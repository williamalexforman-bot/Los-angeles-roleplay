...
test('expanded application and shift panels remain within V2 limits with all emojis installed',()=>{
 ...
 for(const p of [require('../src/applications').applicationPanel(),require('../src/shifts').shiftPanel()]){
  ...
  assert.ok(text.length<=4000);assert.ok(count<=40);assert.match(text,/<:valenti_/);assert.ok(p.flags&D.MessageFlags.IsComponentsV2);
 }
 E.configure(null);
});
