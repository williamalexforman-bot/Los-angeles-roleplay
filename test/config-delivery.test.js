...
test('invalid emoji retry strips dropdown and button emojis and keeps V2 controls',async()=>{
 const {guild,channel}=fixture();let count=0;channel.send=async p=>{if(++count===1)throw {code:50035,rawError:{errors:{components:{_errors:[{message:'Invalid emoji'}]}}}};assert.ok(p.flags&D.MessageFlags.IsComponentsV2);const menu=p.components[0].components.find(c=>c.type===1).components[0];assert.equal(menu.custom_id,'ticket:create');assert.equal(menu.options.length,3);assert.ok(menu.options.every(o=>!o.emoji));return 'sent';};assert.equal(await sendPanel(channel,guild,panel('ticket')),'sent');assert.equal(count,2);
});
