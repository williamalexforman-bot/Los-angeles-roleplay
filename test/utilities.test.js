require('../src/store').collection=()=>({findOne:async()=>({role_management:'manager'})});
...
test('new slash commands serialize and disciplinary notices are V2',()=>{
 ...
 for(const name of ['close','closerequest','ticketpanel'])assert.ok(commands.some(c=>c.name===name));
