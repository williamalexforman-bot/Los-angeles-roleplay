...
test('missing first-tier markers never shift second-tier roles into first tier',()=>{
 const roles=new D.Collection([['w2',{id:'w2',name:'Warning 2'}],['s2',{id:'s2',name:'Strike 2'}]]);
 const found=readInfractionRoles({id:'server',roles:{cache:roles}});
 assert.deepEqual(found.warnings,[null,'w2']);assert.deepEqual(found.strikes,[null,'s2']);
});
