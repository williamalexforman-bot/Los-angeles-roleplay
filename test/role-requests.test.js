...
function fixture(){
 locks=new Map();db={config:[{_id:'g',role_hr:'hr'}]};const sent=[],edited=[],grants=[];let fail=false;
 ...
 const channel={id:CHANNELS.roleRequests,guildId:'g',isTextBased:()=>true,permissionsFor:()=>new D.PermissionsBitField(D.PermissionFlagsBits.Administrator),send:async p=>{sent.push(p);return message;},messages:{fetch:async()=>message}};
...
test('request is saved, posts V2 to HR with a restricted role ping, and suppresses duplicates',async()=>{
 const f=fixture();await R.submit(f.context());assert.equal(db.role_requests.length,1);assert.equal(f.sent.length,1);assert.equal(db.role_requests[0].channelId,'1550639906039136359');assert.deepEqual(f.sent[0].allowedMentions,{parse:[],roles:['hr']});assert.ok(f.sent[0].flags&D.MessageFlags.IsComponentsV2);
