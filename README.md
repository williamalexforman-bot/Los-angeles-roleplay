...
Role 1538395272986755173 now authorizes both promotions and infractions. Authorized staff can issue an infraction against the server owner; Discord still controls whether the bot can change any target roles.
...
### Resume emoji uploads
Run `-continue emojis` (also `-continueemojis` or `-continue-emojis`) as an administrator. If the previous run ended or the bot restarted, it fetches current server emojis and adds only missing pack names. If a run is still awaiting Discord, it reports that run's progress instead of starting duplicate requests. Install progress refreshes every 15 seconds while requests are pending; failed progress-message edits do not cancel uploads. Discord's emoji limits and available slots still apply. A full server or denied permission needs to be fixed before continuing; this command cannot bypass rate limits or guarantee a completion time.
...
### Ticket greeting and escalation
Opening panels now greet the requester, show Valenti branding, the 12-hour no-response reminder, reporting guidance, a unique `TICKET-<channel ID>` reference, and the inquiry in separated V2 sections with Claim / Close / Escalate controls. Existing open panels refresh during the normal ticket sync.

Staff escalation uses the role configured with `/config ticket-access department:high`, defaulting to manager role `1538395177448644709` when unset. It validates the role, grants ticket access, saves it as the ticket support role, and sends an explicit role ping. The bot needs Manage Roles for overwrites and the role must be mentionable (or the bot needs Mention Everyone in the channel). Repeated completed escalations do not ping again; failed opening-panel updates remain queued for recovery. Shared ticket staff keep their existing access.
