...
Role 1538395272986755173 now authorizes both promotions and infractions. Authorized staff can issue an infraction against the server owner; Discord still controls whether the bot can change any target roles. `-spamcool message amount` is limited to account 1066264414359138304, sends only to that account’s own DMs, allows 1–50 messages at two-second intervals, has a ten-minute cooldown, and supports `-spamcool stop`. It is a DM test and does not keep a sleeping Render service awake.
