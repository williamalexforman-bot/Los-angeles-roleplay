...
const PREFIX_COMMANDS = [['-say <message>','Staff: send a message as the bot and remove the successful command invocation.'],['-dm @user <message>','Owner only: send one user a direct message.'],['-role add/remove @user @role','Owner only: change a role when Discord hierarchy permits it.'],['-shift start / break / end / status','Track duty, breaks, and weekly quota.'],['-add emojis','Install or migrate five transparent white panel emojis (Administrator).'],['-continue emojis','Install the next missing batch.'],['-force stop emojis','Stop current and queued emoji work.'],['-deployment','Post a deployment.'],['-lock','Lock the current channel until staff uses -unlock.'],['-unlock','Restore the channel permissions saved by -lock.'],['-removefrom @user','Remove a member from the current ticket.'],['-close','Save the transcript and close a ticket.'],['-closerequest <reason>','Ask the opener to close their ticket.'],['-ticketpanel','Post the configured ticket panel.']];
