...
async function deliver(channel,record,message){
 ...
 for(let attempt=0;;attempt++){
  try {
   const sent=message?await message.edit(payload):await channel.send({...payload,nonce:record._id,enforceNonce:true,allowedMentions:{parse:['everyone'],users:[record.owner],roles:[record.support || TICKET_ACCESS_ROLE].filter(Boolean)}});
