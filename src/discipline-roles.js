...
function normalized(role){return String(role.name || '').toLowerCase().replace(/[^a-z0-9]/g,'');}
...
function readInfractionRoles(guild){
 ...
 ROLES.warnings=[first(roles,[/^warning(?:1|i|one)$/,/^w1$/]),first(roles,[/^warning(?:2|ii|two)$/,/^w2$/])];
 ROLES.strikes=[first(roles,[/^strike(?:1|i|one)$/,/^s1$/]),first(roles,[/^strike(?:2|ii|two)$/,/^s2$/])];
