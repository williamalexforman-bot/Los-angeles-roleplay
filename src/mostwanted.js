...
function notice(account, reason, actor) {
...
async function execute(i, request = fetch, pause) {
  ...
  const message = await channel.send({ ...notice(account, reason, i.user.id), nonce: i.id, enforceNonce: true });
  await i.editReply(v2('Most Wanted Posted', `[View the notice](${message.url}) in <#${CHANNEL_ID}>.`, [], true));
}

module.exports = { CHANNEL_ID, robloxAccount, notice, execute };
