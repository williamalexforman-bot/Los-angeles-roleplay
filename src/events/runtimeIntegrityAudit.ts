...
const TICKET_CATEGORIES = {
    general: '1526254341646712883',
    internal: '1526254402426503320',
    management: '1526254462128099479',
    highrank: '1526254518570844231',
} as const;

const TICKET_ROLES = {
    general: '1523122697746382868',
    internal: '1521593407816990811',
    management: '1523122912201277590',
    highrank: '1527845170748326021',
} as const;

const CRITICAL_CHANNELS: Record<string, string> = {
    ticketPanel: '1526034504953892925',
...
export async function runRuntimeIntegrityAudit(client: Client): Promise<void> {
    ...
    for (const [type, categoryId] of Object.entries(TICKET_CATEGORIES)) {
        ...
        if (!category) {
            issues.push(`ticket category ${type} (${categoryId}) is missing`);
        } else if (category.type !== ChannelType.GuildCategory) {
            issues.push(`ticket category ${type} (${categoryId}) is not a category`);
        }
    }

    for (const [type, roleId] of Object.entries(TICKET_ROLES)) {
        const role = guild.roles.cache.get(roleId) || await guild.roles.fetch(roleId).catch(() => null);
        if (!role) issues.push(`ticket role ${type} (${roleId}) is missing`);
    }
    ...
    if (!me) {
        ...
        if (missingGuildPermissions.length) {
            issues.push(`bot is missing guild permissions: ${missingGuildPermissions.join(', ')}`);
        }
    }
    ...
    logger.info('[IntegrityAudit] PASSED: ticket categories/roles, critical channels, and core bot permissions are valid.');
}
