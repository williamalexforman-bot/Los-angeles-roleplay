...
const staffFeedbackCommand = {
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        try {
            ...
            await interaction.editReply(auditWritten
    ...
    return new ModalBuilder()
        ...
        .addComponents(
            new ActionRowBuilder<TextInputBuilder>().addComponents(new TextInputBuilder()
                .setCustomId('server_name').setLabel('Server name').setPlaceholder('Los Angeles Roleplay').setStyle(TextInputStyle.Short).setMaxLength(100).setRequired(true)),
            new ActionRowBuilder<TextInputBuilder>().addComponents(new TextInputBuilder()
                .setCustomId('representative').setLabel('Server representative').setPlaceholder('Your Discord username').setStyle(TextInputStyle.Short).setMaxLength(100).setRequired(true)),
            new ActionRowBuilder<TextInputBuilder>().addComponents(new TextInputBuilder()
                .setCustomId('invite_link').setLabel('Permanent invite link').setPlaceholder('https://discord.gg/example').setStyle(TextInputStyle.Short).setMaxLength(500).setRequired(true)),
            new ActionRowBuilder<TextInputBuilder>().addComponents(new TextInputBuilder()
                .setCustomId('server_ad').setLabel('Server advertisement').setPlaceholder('Tell us about your server and community.').setStyle(TextInputStyle.Paragraph).setMaxLength(4000).setRequired(true)),
        );
}

function partnershipReviewComponents(submitterId: string, disabled = false): ActionRowBuilder<ButtonBuilder>[] {
    return [new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId(`partnership:approve:${submitterId}`).setLabel('Approve').setEmoji('✅').setStyle(ButtonStyle.Success).setDisabled(disabled),
        new ButtonBuilder().setCustomId(`partnership:deny:${submitterId}`).setLabel('Deny').setEmoji('❌').setStyle(ButtonStyle.Danger).setDisabled(disabled),
    )];
}
...
function partnershipDetails(data: PartnershipRequestData, status: 'Pending Review' | 'Approved' | 'Denied', reviewerId?: string): string {
    return [
...
function reviewedPartnershipComponents(message: Message, approved: boolean, reviewerId: string): unknown[] {
    ...
    const visit = (node: PartnershipComponentNode): void => {
        if (typeof node.content === 'string' && node.content.includes('> **Status:**')) {
            ...
            if (node.content.startsWith('## ')) node.content = node.content.replace(/^## .+/, `## ${approved ? '✅ Partnership Approved' : '❌ Partnership Denied'}`);
        }
...
const partnershipCommand = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand => subcommand.setName('request').setDescription('Post the professional partnership request panel in this channel')),
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        await interaction.deferReply({ flags: MessageFlags.Ephemeral });
        ...
        await destination.send({
            ...
            files: partnershipArtworkAttachments(),
...
const staffComplaintCommand = {
    data: new SlashCommandBuilder()
        ...
        .addUserOption(option => option.setName('member').setDescription('The staff member you are reporting').setRequired(true))
        .addIntegerOption(option => option.setName('rating').setDescription('Your rating from 1 to 5').setMinValue(1).setMaxValue(5).setRequired(true))
        .addStringOption(option => option.setName('what').setDescription('What did the staff member do?').setMaxLength(4000).setRequired(true)),
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        await interaction.deferReply({ flags: MessageFlags.Ephemeral });
        try {
            ...
            const complaintEmbed = brandedEmbed('📋 Staff Complaint Received', undefined, 0xef4444).addFields(
            ...
            await destination.send(legacyEmbedToV2Message(complaintEmbed, { allowedMentions: { parse: [] } }));
