        ...
        try {
            ...
            if (subcommand === 'view') {
                if (!interaction.guildId || !isDatabaseAvailable()) {
                    await interaction.editReply('The database is currently unavailable. Cannot view activity checks.');
                    return;
                }
            ...
            if (subcommand === 'end') {
                if (!interaction.guildId || !isDatabaseAvailable()) {
                    await interaction.editReply('The database is currently unavailable. Cannot end activity check.');
                    return;
                }
