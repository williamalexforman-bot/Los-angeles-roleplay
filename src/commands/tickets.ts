    ...
    return [
        new ActionRowBuilder<ButtonBuilder>().addComponents(
            new ButtonBuilder()
                ...
                .setLabel(claimedBy ? `Claimed by ${claimedBy}`.slice(0, 80) : 'Claim Ticket')
                .setEmoji('🙋')
...
function buildOpenTicketPanel(
    ...
    const details = [
        ...
        `### ${discordLogo} Discord Account Information`,
        `> **Username:** \`${compact(username, 80)}\``,
        `> **User ID:** \`${userId}\``,
        `> **Account Created:** <t:${Math.floor(accountCreated / 1_000)}:F>`,
        `> **Joined Server:** ${joinedAt ? `<t:${Math.floor(joinedAt / 1_000)}:F>` : 'Unavailable'}`,
    ].join('\n');
    ...
    const panel = new ContainerBuilder()
        ...
        .addMediaGalleryComponents(media(ASSISTANCE_BANNER_NAME))
        .addSeparatorComponents(separator())
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(details.slice(0, 4_000)));
...
function isTicketStaff(interaction: ButtonInteraction | ModalSubmitInteraction | ChatInputCommandInteraction): boolean {
...
function createTicketModal(type: TicketType): ModalBuilder {
    const modal = new ModalBuilder()
        .setCustomId(`ticket:create-modal:${type}`)
...
async function discordLogo(guild: ModalSubmitInteraction['guild']): Promise<string> {
    if (!guild) return '💬';
    const emojis = await guild.emojis.fetch().catch(() => null);
    return emojis?.find(emoji => emoji.name?.toLowerCase() === 'discord_logo')?.toString() || '💬';
}

function modalAnswers(interaction: ModalSubmitInteraction, type: TicketType): Array<{ label: string; value: string }> {
    if (type === 'internal') {
        return [
    ...
    try {
        ...
        await channel.send({
            components: [buildOpenTicketPanel(
                ...
                type,
                modalAnswers(interaction, type),
                await discordLogo(guild),
            )],
            files: artwork(),
...
async function ticketTranscript(channel: TextChannel): Promise<Buffer> {
    ...
    const lines = messages
        ? Array.from(messages.values())
            ...
            .map(message => {
                ...
                return `[${message.createdAt.toISOString()}] ${message.author.tag} (${message.author.id}): ${message.cleanContent || '[no text]'}${attachments ? ` ${attachments}` : ''}`;
            })
        : ['Transcript could not be fetched.'];
    return Buffer.from(lines.join('\n'), 'utf8');
}

async function writeCloseLogs(
    ...
    if (logChannel?.isSendable()) {
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Category', value: TICKET_CATEGORIES[metadata.type].label, inline: true },
                { name: 'Claimed By', value: metadata.claimedBy ? `<@${metadata.claimedBy}>` : 'Unclaimed', inline: true },
                { name: 'Reason', value: compact(reason, 1_000) },
            )
...
async function closeTicket(
    ...
    await writeCloseLogs(interaction, channel, metadata, reason);
    await interaction.editReply('🔒 Ticket closed. The transcript and ticket log were saved.');
    await channel.delete(`Ticket closed by ${interaction.user.tag}: ${reason}`).catch(error => {
        logger.error(`[Tickets] Could not delete ${channel.id}: ${error instanceof Error ? error.message : 'Unknown'}`);
    });
}
...
export async function handleTicketButton(interaction: ButtonInteraction): Promise<boolean> {
    ...
    if (interaction.customId === 'ticket:close') {
        await interaction.deferReply({ flags: MessageFlags.Ephemeral });
        await closeTicket(interaction, 'Closed from the ticket panel.');
        return true;
    }
    ...
    if (interaction.customId === 'ticket:close-confirm') {
        await interaction.deferReply({ flags: MessageFlags.Ephemeral });
        await closeTicket(interaction, 'Close request accepted.');
        return true;
    }
...
const closeCommand = {
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        await interaction.deferReply({ flags: MessageFlags.Ephemeral });
        await closeTicket(interaction, 'Closed with /close.');
    },
};
