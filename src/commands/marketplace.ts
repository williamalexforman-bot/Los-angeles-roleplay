...
const MARKETPLACE_BANNER_NAME = 'marketplace-banner.webp';
...
const MARKETPLACE_ITEMS: readonly MarketplaceItem[] = [
    {
        id: 'paid-ad-everyone',
        title: 'Paid Ad — @everyone',
        description: 'With an everyone ping, everyone on the server will be notified of your advertisement.',
        price: 450,
    },
    {
        id: 'paid-ad-here',
        title: 'Paid Ad — @here',
        description: 'With a here ping, everyone currently online in the server will be notified of your advertisement.',
        price: 450,
    },
    {
        id: 'giveaway-everyone',
        title: 'Sponsored Giveaway — @everyone',
        description: 'With an everyone ping, every online member in the server will be notified of your giveaway.',
        price: 650,
    },
    {
        id: 'giveaway-here',
        title: 'Sponsored Giveaway — @here',
        description: 'With a here ping, every online member in the server will be notified of your giveaway.',
        price: 650,
    },
    {
        id: 'priority-queue',
        title: 'Priority Queue',
        description: 'With priority, your advertisement is transferred to the priority queue.',
        price: 800,
    },
    {
        id: 'plus',
        title: 'Plus',
        description: 'You gain access to Exclusive Channels, a Hoisted Role (Plus), GIF/Embed Permissions, Media Permissions (Attaching Files), Add Reactions, Use of external emojis & stickers, and $30,000 in Economy Money every 1 hour.',
        price: 800,
    },
];
...
function priceButton(item: MarketplaceItem): ButtonBuilder {
    return new ButtonBuilder()
        .setCustomId(`marketplace:price:${item.id}`)
        .setLabel(`${item.price}`)
        .setEmoji('💠')
...
function marketplaceItemSection(item: MarketplaceItem): SectionBuilder {
    return new SectionBuilder()
        ...
        .setButtonAccessory(priceButton(item));
}
    ...
    const panel = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            new TextDisplayBuilder().setContent(
                '## Marketplace\n'
                + 'We have a marketplace that allows you to support us and receive something in return! '
                + 'All purchases are final, with no refunds being made under any circumstances. '
                + 'Chargeback fraud relating to USD purchases will result in a permanent ban.',
            ),
        );

    for (const item of MARKETPLACE_ITEMS) {
        panel.addSectionComponents(marketplaceItemSection(item));
    }

    return panel
        .addActionRowComponents(
            new ActionRowBuilder<ButtonBuilder>().addComponents(claimButton),
        )
...
export const marketplacePanelCommand = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            await interaction.editReply('I could not post the marketplace panel. Check my channel permissions and make sure the marketplace banner asset exists.');
        }
    },
};
