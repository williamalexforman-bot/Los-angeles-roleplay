...
async function syncCasePanels(client){
 const cases=await collection('cases').find({status:'logged',noticeVersion:{$ne:2},messageId:{$exists:true}}).limit(50).toArray();
 for(const item of cases){try{await locked(`member:${item.guildId}:${item.userId}`,async()=>{
  const fresh=await collection('cases').findOne({_id:item._id});if(!fresh||fresh.noticeVersion===2)return;
  ...
  await collection('cases').updateOne({_id:fresh._id},{$set:{noticeVersion:2}});
