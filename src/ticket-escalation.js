...
async function escalate(i,record){
 if(record.escalationNotified)return 'This ticket has already been escalated to Senior High Rank.';
 const config=await settings(i.guildId);
 const roleId=config.support_high || '1538395177448644709';
 const role=await i.guild.roles.fetch(roleId);
 if(!role || role.id===i.guildId)throw new Error('Set a valid Senior High Rank role using /config ticket-access department:high.');
 ...
 return 'Ticket escalated to Senior High Rank. The role has access and has been notified.';
}
